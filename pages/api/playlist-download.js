...
export default async function handler(req, res) {
  ...
  try {
    const backendUrl = buildBackendUrl('/playlist-download', req.query);
    ...
    console.error('[api/playlist-download] error:', err);
    return res.status(503).json({
      error: 'Playlist download server is unavailable. Start server.py, then try again.',
    });
  }
}
